...
- Opus: ゲスト固有の事実→具体的な編集手順まで落とし込める品質
- 3000文字制限撤廃: セクション5全文（NGパターン・完成品事例含む）参照OK
- プロファイル制限撤廃: 職業・年収・実績が具体的に反映
- コンテンツライン注入: キャリア軸→社格重視 / 不動産軸→実績数字重視と正しく分岐

### 2026-03-20 完了タスク（セッション3: FB変換品質改善）

| # | タスク | 状態 |
|---|--------|------|
| 26 | Phase 5: 品質チェック（29件突合、83%正解率、重大問題4件特定） | ✅ |
| 27 | quality_knowledge_loader.py 新規作成（品質基準プロンプト注入基盤） | ✅ |
| 28 | コンテンツライン自動判定実装（キャリア軸/不動産軸） | ✅ |
| 29 | FB変換: model=sonnet→opus（MAX定額内Opus適用） | ✅ |
| 30 | FB変換: 3000文字切り詰め制限撤廃（セクション5全文10,698文字注入） | ✅ |
| 31 | FB変換: ゲストプロファイル1500文字制限撤廃 | ✅ |
| 32 | FB変換: project_id→コンテンツライン注入（不動産軸/キャリア軸） | ✅ |
| 33 | プロファイルパーサー動作確認（hirai=層a, さるビール=層a, 真生=層c — 現コード正常） | ✅ |
| 34 | テスト77件全PASS確認 | ✅ |
| 35 | Mac1ゾンビプロセス掃除（145本→7本、load average回復） | ✅ |

### 未着手・次のアクション
1. **改善後の品質確認**: 問題だった4-5件でFB変換を再実行し品質差を確認
2. **承認フロー設計**: FB変換結果を「承認待ち」で保持→アプリ上で承認/修正/却下
3. **概要欄未生成11件/29件(38%)**: Phase 5で発見された問題の調査
4. **タイミング②生成時品質注入**: Phase 5品質チェック結果の反映
5. **タイミング③編集後自動QC**: 全部新規（①②が固まった後）
6. **事務依頼送信**: チャンネル変更+命名規則+メインカメラ再アップ（優先度低）

### 設計確定事項（2026-03-20 なおとさんとの壁打ち）
- FB承認フロー: **事前承認制**（なおとさん/パグさんが各々承認してから編集者に流す）
- FBスタイル: **1コメント1テーマ**（複合FB分解は不要）
- インフラ: Mac2/Mac3は夜まで外出中のため後回し

### 2026-03-19 完了タスク（セッション2: ディレクションレポート品質改善）

| # | タスク | 状態 |
|---|--------|------|
| 15 | Phase 1: ゲスト層分類ロジック根本改修（年収1000万ハードコード→マニュアル基準5段階判定） | ✅ |
| 16 | TIER_A_KEYWORDSにリクルート・花王・キリン・LINEヤフー・パナソニック等の日系超大手追加 | ✅ |
| 17 | Phase 2: プロファイル情報フォールバック抽出（メタ情報セクションから抽出、ハイライトセクション名バリエーション対応） | ✅ |
| 18 | Phase 3: 演出ディレクションのテンプレ抽象指示→具体的編集者向け指示に改修 | ✅ |
| 19 | 相槌フィルタリング追加（_is_filler関数） | ✅ |
| 20 | 層別カラーコード具体指定（層a=ゴールド、層b=ブルー） | ✅ |
| 21 | LLMプロンプトの層別演出方針をマニュアル完全準拠に修正（年収レンジ矛盾解消） | ✅ |
| 22 | 品質判断ガイド作成（docs/QUALITY_JUDGMENT_GUIDE.md — なおとさんFB全反映） | ✅ |
| 23 | 起動時バナー抑制（hasEverConnected修正） | ✅ |
| 24 | レビューコメントデコードエラー修正（.convertFromSnakeCase+CodingKeys競合）| ✅ |
| 25 | Phase 4: 全29件一括再生成バッチ | 実行中 |

### 2026-03-19 完了タスク（セッション1: DNS・通信・素材連携）

| # | タスク | 状態 |
|---|--------|------|
| 1 | DNS根本解決（legit-marc.com NS変更: Xserver → Cloudflare） | ✅ |
| 2 | cloudflaredトンネル経由で4G/5GからTailscale不要でAPI接続可能に | ✅ |
| 3 | cloudflared永続化（launchd登録確認、手動プロセス→launchctl管理に統一） | ✅ |
| 4 | 3者体制通信監査: Codex CLI（Python側）Critical 2件 + Moderate 4件検出 | ✅ |
| 5 | 3者体制通信監査: GPT-5.4（設計レビュー）mission-dispatch経由で実行 | ✅ |
| 6 | バティ右腕Python修正実装: 全6項目修正完了 | ✅ |
| 7 | パグさんTestFlight内部テスター再招待（期限切れ → 再招待完了） | ✅ |
| 8 | 素材動画連携: 松本さん2動画 + スリマンさん1動画 連携完了 | ✅ |
| 9 | 素材動画: あさかつさん2 音声判定ミス誤登録 → 削除済み | ✅ |
| 10 | 素材動画: 未連携13件の原因特定（YouTube→アプリ自動連携が未実装） | ✅ |
| 11 | 音声判定閾値修正: mean > -55dB かつ max > -30dB に厳格化 | ✅ |
| 12 | 事務への依頼文作成（チャンネル変更+命名規則+メインカメラ再アップ） | ✅ |
| 13 | gitビルドアーティファクト除去: build/配下12ファイルをgit管理から除去 | ✅ |
| 14 | ビルド30作成 → TestFlightアップロード完了（通信修正反映済み） | ✅ |

**3者体制通信監査 Python側修正の詳細（全6項目）:**
- CORS `*` → localhost明示列挙 + 環境変数対応
- `/healthz` + `/readyz` 追加
- `/api/health` から db_path 削除
- SQLite timeout/busy_timeout/WAL追加
- 生例外文字列のクライアント露出防止（6箇所）
- 共通エラーハンドラ（統一JSON形式）
- Swift側: 前セッションで全6項目修正済みを確認、BUILD SUCCEEDED

### 未完了の作業
| # | タスク | 状態 |
|---|--------|------|
| 1 | **Phase 5: 品質チェック（マニュアル基準で全件突合）** | Phase 4完了後 |
| 2 | **AI生成物への音声FB・自己成長設計** | Phase 5完了後に着手（なおとさんOK済み） |
| 3 | **TestFlightビルド→アップロード** | Phase 5完了後 |
| 4 | 素材動画自動連携の仕組み実装（新チャンネル確定後） | 待ち |
| 5 | statusAPIバグ修正（linked_countがレガシーカラムだけ見ている） | 未着手 |
| 6 | サムネディレクション品質向上 | 後回し（なおとさん指示） |
| 7 | くますけ・アンディ・ロキ・あさかつ1&2のメインカメラ再アップ待ち | 待ち |
| 8 | git履歴からのバイナリ完全除去（.gitが131MB） | 未着手 |

### 次にやるべき作業（優先順位付き）
| # | タスク | 優先度 |
|---|--------|--------|
| 1 | 事務への依頼送信（チャンネル変更+命名規則+メインカメラ再アップ） | P0 |
| 2 | 新チャンネル確定後、素材動画自動連携の仕組み実装 | P1 |
| 3 | statusAPIバグ修正 | P1 |
| 4 | Moderate修正の残り | P2 |
...
| # | 問題 | 状態 |
|---|------|------|
| 1 | 退職者が作ったGoogleアカウントの素材管理チャンネル → 2段階認証パスできない → 新チャンネルに移行予定 | 対応中 |
| 2 | 音声判定の閾値（mean > -55dB, max > -30dB）は暫定値、サンプル数が少ないため今後調整の余地あり | 経過観察 |
| 3 | auto-syncが3分間隔で動いてるため、コミットメッセージが汎用的になりがち | 運用上の課題 |
