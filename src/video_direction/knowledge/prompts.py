...
【最重要】full_textにはそのままコピペして使える概要欄の全文を必ず入れること。テンプレート固定部分（CTA・運営者情報等）は description_writer.py 側で付与するため不要。AIが生成すべきは、ゲスト紹介フック・トークサマリー・ハッシュタグ・タイムスタンプの部分。full_textを空にすることは絶対に禁止。

以下のJSON形式で出力してください:
```json
{{
  "full_text": "そのままコピペして使える概要欄の全文テキスト（必須・空文字禁止）",
  "sections": {{
