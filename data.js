  ...
  projects: [
    {
      ...
      knowledge: {
        ...
        fullTranscript: `冒頭で意思決定の基準を先に置くと、視聴者はこの対談をどう見ればいいかがすぐ分かる。
そのうえで具体事例に入ると、抽象論ではなく再現可能な判断軸として受け取られやすい。
中盤の事例整理では、結論→補足→具体例の順で並べると視聴維持が安定する。`
      },
      vimeoReview: {
        url: '#vimeo-vd-001',
    ...
    },
    {
      ...
      knowledge: {
        ...
        fullTranscript: `採用現場の温度感はかなり強いので、最初の数カットで現場の空気を見せ切った方が視聴者が入りやすい。
そのあとに情報へ入ることで、単なる説明ではなく現場の熱量を伴ったメッセージになる。
音声の抜けを整えると、言葉の説得力も一段上がる。`
      },
      vimeoReview: {
        url: '#vimeo-vd-002',
    ...
    },
    {
      ...
      knowledge: {
        ...
        fullTranscript: `ブランドムービーでは世界観の強さが重要だが、視聴者が判断できる具体要素を一つ置くと理解が深まる。
冒頭で何を感じてほしいかを先に定義し、その後に映像体験を重ねると印象が残りやすい。
情緒だけで押し切らず、判断材料を一点置くことでブランドの輪郭が立つ。`
      },
      vimeoReview: {
        url: '#vimeo-vd-003',
        statusLabel: '確認済み',
    ...
    },
    {
      ...
      knowledge: {
        ...
        fullTranscript: `イベントダイジェストは全体の雰囲気だけでなく、人の熱量が見える寄りカットを差すと記憶に残る。
冒頭で会場全体の空気を掴ませ、そのあとに個人の表情や反応を差し込むと熱量が伝わる。
ハイライトの切り出しは、全体と個人の往復で構成すると強い。`
      },
      vimeoReview: {
        url: '#vimeo-vd-004',
        statusLabel: '公開完了',
    ...
    },
    {
      ...
      knowledge: {
        ...
        fullTranscript: `入門動画は情報密度よりも理解速度が大事なので、章立てと結論先出しが効く。
最初に何が分かる動画かを言い切ると、視聴者が迷わず追いやすい。
細かい説明は後ろに回して、まず全体像を渡す構成が理解負荷を下げる。`
      },
      vimeoReview: {
        url: '#vimeo-vd-005',
        statusLabel: '公開完了',
