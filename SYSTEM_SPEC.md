...
│   ├── analyzer/                       ← 分析（LLM呼び出し4モジュール含む）
│   │   ├── direction_generator.py (723行) ← ⚠️ Sonnet（Opus統一すべき）。演出ディレクション
│   │   ├── description_writer.py (603行)  ← ⚠️ Sonnet（Opus統一すべき）。概要欄生成
│   │   ├── title_generator.py (290行)     ← ⚠️ Sonnet（Opus統一すべき）。タイトル案生成
│   │   ├── thumbnail_designer.py (189行)  ← ⚠️ Sonnet（Opus統一すべき）。サムネ設計指示書
