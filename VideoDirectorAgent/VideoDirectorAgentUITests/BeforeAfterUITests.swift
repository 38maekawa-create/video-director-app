...
final class BeforeAfterUITests: XCTestCase {
    ...
    func testDirectionReportBeforeAfterButtonOpensSummaryWithoutCrash() throws {
        ...
        XCTAssertTrue(app.otherElements["before-after-two-up-comparison"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.otherElements["before-after-two-up-top"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.otherElements["before-after-two-up-bottom"].waitForExistence(timeout: 20))
