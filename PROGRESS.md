...
| # | タスク | 状態 |
|---|--------|------|
| 1 | 素材動画自動連携の仕組み実装（新チャンネル確定後にYouTube Data API + OAuth連携構築） | 待ち |
| 2 | statusAPIバグ修正（linked_countがレガシーカラムprojects.source_videoだけ見ている） | 未着手 |
| 3 | くますけ・アンディ・ロキ・あさかつ1&2のメインカメラ再アップ待ち（事務に依頼予定） | 待ち |
| 4 | コテツ・kos・メンイチ・さといもトーマス・ハオの素材動画（2/28大阪撮影分、YouTubeチャンネルに動画なし） | 待ち |
| 5 | git履歴からのバイナリ完全除去（.gitが131MB、git filter-repo検討） | 未着手 |
| 6 | Moderate修正13件 + bare except 20件（Python側の残り改善項目） | 未着手 |
