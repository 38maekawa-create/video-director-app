...
### 1-1. 目的
映像作品全般の品質を追求し、永遠に自己成長し続けるシステム。外部映像（YouTube・Netflix・Hulu等）をトラッキング・分析し、「質の良い映像とは何か」を追求して蓄積されたナレッジを元に、映像制作のあらゆる局面で品質向上を支援する。

TEKO対談インタビュー動画の編集者向けディレクションレポート自動生成は、このシステムの**機能の一部**として位置づけられる。

**用語の使い分け**: 「映像」= AI開発10（映像品質追求）、「動画」= AI開発5（動画ナレッジ・コンテンツ目的）。意図的に使い分けている。

### 1-2. ユーザー体験（完成形）

**コア体験: 映像品質の自己成長サイクル**
1. 外部映像（YouTube・Netflix等）をトラッキング → 演出・デザイン・品質軸で分析・要素分解
2. 人間（なおと・パグさん）のFBを蓄積 → 「良い映像とは」の基準を継続学習
3. 蓄積された知見がディレクション・品質評価のあらゆる機能に自動反映

**TEKO対談ディレクション体験（機能の一部）:**
