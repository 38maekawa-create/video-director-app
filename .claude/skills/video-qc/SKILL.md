...
- [ ] ゲスト情報（名前・肩書き・実績）が正確か（member_master.py経由でMEMBER_MASTER.jsonと照合。JSON不在時は生成物内の情報を仮扱いとし「ナレッジ未確認」と報告）
...
3. 概要欄の修正:
   - 修正対象の優先順位: `.data/` 内の生成物ファイル → `output/` 配下のレポート
   - 公開済みYouTube概要欄は直接変更しない。差分案を出してなおとさん確認
   - 修正理由は KNOWN_CORRECTIONS.md に記録（出力ファイル本文にコメントを混入させない）
4. 層分類の修正:
